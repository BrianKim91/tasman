...
//*************************************************************
	...
	function create_range_select(){
			connect_mysql();
			$sql="SELECT `id`, `pbrand` FROM `brand_range`" ; 
			$result=mysql_query($sql);
			while ($rs=mysql_fetch_array($result)) {
				echo "<option value='".$rs['id']."'>".ucwords($rs['pbrand'])."</option>";
			}
		}
	...
	<body>
		...
		<fieldset>
		    <legend><strong>Add Products brand Range</strong></legend> 
			Products brand Range:  <input type="text" name="range_brand" required>
			<input type="submit" name="brand_add" style="width:100px;" value="Add">
		</fieldset>
	</form>
		<br><br>
		<!--********************************************************************-->
		...
		<fieldset>
		    ...
		    <table>
		    	<tr><td>Product Brand: </td><td><select name="prange">
		    		<?php create_range_select();?></select> </td></tr>
		    	<tr><td>Product name: </td><td><input type="text" name="pname" required></td></tr>
		    	<tr><td>Product price:</td><td><input type="text" name="pprice" required></td></tr>
		    	<tr><td>Product detail:</td><td><textarea cols="20" rows="10" type="text" name="pdetail" required></textarea></td></tr>
		    	
		    	<tr><td colspan="2" style="text-align: center;"><input style="width:100px;" type="submit" name="product_add" value="Add"></td></tr>
		    </table>		
		</fieldset>
	</form> 
	</div>
	...
	<!--*******************************************************************************************************-->
<?php	
		function create_range_select1($selected_id){
			connect_mysql();
			$sql="SELECT `id`, `pbrand` FROM `brand_range`" ; 
			$i=0;
			$result=mysql_query($sql);
	...
	function select(obj){
	window.location.href='admin.php?id='+obj.options[obj.selectedIndex].value;
	return; 
	}
	</script>
	</head>
		...
		<fieldset>
		    ...
		    <table class="myTable" style="border-bottom: : 0px solid black;">
		    	...
		    	<tr><td colspan="3" style="text-align: right;" >Product Brand range: </td><td colspan="1" style="text-align: right;" ><select name="prange" onchange="select(this)">
		    		...
		    		<?php create_range_select1($prange);?>
		    ...
		    <tr><th  style="width:50px;">ID</th><th style="width:100px;">Name</th><th style="width:300px;">Detail</th><th>Price($)</th></tr>
		    	<?php
						...
						$result=mysql_query($sql);
						while ($rs=mysql_fetch_array($result)) {
							echo "<tr><td>".$rs['id']."</td><td><a href='admin.php?id=".$prange."&pid=".$rs['id']."'>".ucwords($rs['pname'])."</a></td><td>".ucwords($rs['pdetail'])."</td><td>".ucwords($rs['pprice'])."</td></tr>";
						}
					}
					else
					{
